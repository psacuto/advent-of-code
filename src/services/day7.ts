...
class Dir {
  ...
  public getFullSize(): number {
    if (!this.fullSize) {
      this.fullSize =
        this.files.reduce((acc, f) => acc + f.size, 0) +
        this.dirs.reduce((acc, subdir) => acc + subdir.getFullSize(), 0);
    }
...
export function day7_1(input: string) {
  ...
  const sum = rootDir
    .findBy((d) => d.getFullSize() <= 100000)
    .reduce((acc, d) => acc + d.getFullSize(), 0);
...
export function day7_2(input: string) {
  ...
  let result = rootDir
    .findBy((d) => d.getFullSize() + freeSpace >= requiredSpace)
    .map((d) => d.getFullSize())
    .sort((s1, s2) => s1 - s2)[0];
